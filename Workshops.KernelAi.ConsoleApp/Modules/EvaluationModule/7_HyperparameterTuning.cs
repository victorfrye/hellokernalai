...
public class HyperparameterTuning(IAnsiConsole console, WorkshopSettings settings) : IExample
{
    public string Name => "Hyperparamter Tuning";
...
#pragma warning disable AIEVAL001 // RelevanceTruthAndCompletenessEvaluator is still in preview
    public async Task RunAsync()
    {
        ...
        float[] temperaturs = [0.1f, 0.5f, 1f, 2f];
        foreach (float temperature in temperaturs)
        {

            console.Write(new Rule($"Temperature {temperature}"));
            ...
            await foreach (var update in chatClient.GetStreamingResponseAsync(userInput, chatOptions))
            {
            ...
            await using ScenarioRun run =
                ...
                iterationName: temperature.ToString(), 
                additionalTags: [$"Model_{chatSettings.Model}"]);
            await run.EvaluateAsync(userInput, reply);
